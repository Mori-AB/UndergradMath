...
The difference between \(\leftrightarrow\) and \(\iff\)
is that \(\leftrightarrow\) is a connective,
while \(\iff\) is a sentence.
Of course, the distinction is not crucial,
since if \(A\iff B\) for propositions \(A\) and \(B\),
then \(A\leftrightarrow B\) is true for all interpretations.

The first real usage we will make of semantic equality
is for \(n\)-ary conjunctions and disjunctions;
that is, conjunctions and disjunctions for \(n\) variables.

\begin{proposition}[\(n\)-ary Conjunctions and Disjunctions]
    \label{prop:nwedgevee}
    Let \(\{A_1,\ldots,A_n\}\) be a set of propositional variables.
    Then the following hold:
    \begin{itemize}
        \item For any \(1\le i,j,k\le n\),
        \((A_i\wedge A_j)\wedge A_k\iff A_i\wedge(A_j\wedge A_k)\).
        \item For any \(1\le i,j\le n\),
        \(A_i\wedge A_j\iff A_j\wedge A_i\).
        \item For any \(1\le i,j,k\le n\),
        \((A_i\vee A_j)\vee A_k\iff A_i\vee(A_j\vee A_k)\).
        \item For any \(1\le i,j\le n\),
        \(A_i\vee A_j\iff A_j\vee A_i\).
    \end{itemize}
